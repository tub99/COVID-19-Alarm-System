...
class MapVisualiser extends React.Component {
  ...
  initMap(data) {
    ...
    let mapContainerWidth = document.getElementById('map').clientWidth - 20,
      scaleValue = document.body.clientWidth < 500 ? 300 : 800;

    let map = this.stateMap(states.features).width(mapContainerWidth).height(mapContainerWidth).scale(mapContainerWidth);
